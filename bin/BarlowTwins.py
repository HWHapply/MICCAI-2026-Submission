...
class BarlowTwins(pl.LightningModule):
    """
    Disentangled Barlow Twins with Sensitivity-based Disentanglement.
    ...
    - Active group: learns INVARIANCE to its augmentation (via Barlow Twins loss)
    ...
    Loss functions:
    - L_active: Barlow Twins on active group → INVARIANT (updates backbone + head)
    - L_semantic: Barlow Twins on semantic group → INVARIANT to all (updates backbone + head)
    ...
    def __init__(
        ...
        lambd: float = 0.005,  # Weight for off-diagonal terms in cross-correlation matrix
        ...
        ortho_weight: float = 0.01,  # Reduced from 0.1 since Barlow Twins already decorrelates
        ...
        if use_disentanglement:
            ...
            print(f"\n{'='*70}")
            print(f"TRUE DISENTANGLED BARLOW TWINS WITH GRADIENT CONTROL")
            print(f"{'='*70}")
            ...
            print(f"\nLoss Functions:")
            print(f"  - L_active: Barlow Twins (lambda={lambd})")
            print(f"  - L_semantic: Barlow Twins (lambda={lambd})")
            ...
            # Standard Barlow Twins: single projection head
            self.projection_head = ProjectionHead(encoder_dim, encoder_dim, projection_dim)
            print(f"✓ Standard Barlow Twins (no disentanglement)")
    ...
    def forward(self, x, active_group_idx=None, detach_inactive=True):
        ...
        Disentanglement design:
            - Active group: learns INVARIANCE via Barlow Twins loss (updates backbone)
        ...
        if self.use_disentanglement:
            ...
            # NOTE: Barlow Twins normalizes along batch dim inside its loss function
            # No L2 normalization here - let the loss handle normalization
    ...
    def barlow_twins_loss(self, z_i, z_j):
        """
        Barlow Twins loss based on cross-correlation matrix.

        The loss encourages:
        1. Diagonal elements -> 1 (invariance to augmentation)
        2. Off-diagonal elements -> 0 (redundancy reduction)
        ...
        z_i_norm = (z_i - z_i.mean(dim=0)) / (z_i.std(dim=0) + 1e-8)
        z_j_norm = (z_j - z_j.mean(dim=0)) / (z_j.std(dim=0) + 1e-8)

        # Cross-correlation matrix [dim, dim]
        c = torch.mm(z_i_norm.T, z_j_norm) / batch_size

        # Loss: minimize difference from identity matrix
        # Diagonal terms: (c_ii - 1)^2 -> encourages invariance
        # Off-diagonal terms: c_ij^2 -> encourages redundancy reduction
    ...
    def sensitivity_loss(self, z1_groups, z2_groups, active_idx):
        ...
        - Active group: INVARIANT to its aug (via Barlow Twins)
        - Inactive groups: SENSITIVE to active aug (via this loss)

        Requires high --inactive_weight (e.g., 2.0) to compete with Barlow Twins losses.
        """
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_disentanglement:
            ...
            # === LOSS 1: Active group Barlow Twins (using single-aug views) ===
            loss_active = self.barlow_twins_loss(z1_groups[active_idx], z2_groups[active_idx])

            # === LOSS 2: Semantic group Barlow Twins (using ALL-aug views) ===
            semantic_idx = self.num_specific_groups  # Last group is semantic
            loss_semantic = self.barlow_twins_loss(z_sem1_groups[semantic_idx], z_sem2_groups[semantic_idx])
            ...
            # Active group gets uniformity from Barlow Twins (off-diagonal terms)
            # Semantic group gets uniformity from Barlow Twins (L_semantic)
            ...
            # Primary loss for monitoring (sum of Barlow Twins losses)
            ...
            total_loss = self.barlow_twins_loss(z_i, z_j)

            self.log('train/loss', total_loss, on_step=True, on_epoch=True, prog_bar=True, batch_size=batch_size)

            with torch.no_grad():
                # Monitor cross-correlation matrix statistics
                z_i_norm = (z_i - z_i.mean(dim=0)) / (z_i.std(dim=0) + 1e-8)
                z_j_norm = (z_j - z_j.mean(dim=0)) / (z_j.std(dim=0) + 1e-8)
                c = torch.mm(z_i_norm.T, z_j_norm) / batch_size

                diag_mean = torch.diagonal(c).mean()
                off_diag = c.flatten()[:-1].view(c.shape[0] - 1, c.shape[0] + 1)[:, 1:]
                off_diag_mean = off_diag.abs().mean()

                self.log('monitor/cross_corr_diag', diag_mean, on_step=False, on_epoch=True, batch_size=batch_size)
                self.log('monitor/cross_corr_off_diag', off_diag_mean, on_step=False, on_epoch=True, batch_size=batch_size)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.use_disentanglement:
            ...
            with torch.no_grad():
                ...
                self.log('val_monitor/cross_corr_diag', diag_mean, on_step=False, on_epoch=True, batch_size=batch_size)
                self.log('val_monitor/cross_corr_off_diag', off_diag_mean, on_step=False, on_epoch=True, batch_size=batch_size)
