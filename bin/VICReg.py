...
class ProjectionHead(nn.Module):
    """MLP projection head for VICReg."""
...
class VICReg(pl.LightningModule):
    """
    Disentangled VICReg with Sensitivity-based Disentanglement.
    ...
    - Active group: learns INVARIANCE to its augmentation (via VICReg loss)
    ...
    Loss functions:
    - L_active: VICReg on active group → INVARIANT (updates backbone + head)
    - L_semantic: VICReg on semantic group → INVARIANT to all (updates backbone + head)
    ...
    def __init__(
        ...
        sim_loss_weight: float = 25.0,  # Invariance loss weight
        var_loss_weight: float = 25.0,  # Variance loss weight
        cov_loss_weight: float = 1.0,   # Covariance loss weight
        variance_threshold: float = 1.0,  # Minimum variance to maintain
        ...
        ortho_weight: float = 0.01,  # Reduced from 0.1 since VICReg already decorrelates
        ...
        if use_disentanglement:
            ...
            print(f"\n{'='*70}")
            print(f"TRUE DISENTANGLED VICREG WITH GRADIENT CONTROL")
            print(f"{'='*70}")
            ...
            print(f"\nLoss Functions:")
            print(f"  - L_active: VICReg (sim={sim_loss_weight}, var={var_loss_weight}, cov={cov_loss_weight})")
            print(f"  - L_semantic: VICReg (sim={sim_loss_weight}, var={var_loss_weight}, cov={cov_loss_weight})")
            ...
            # Standard VICReg: single projection head
            self.projection_head = ProjectionHead(encoder_dim, encoder_dim, projection_dim)
            print(f"✓ Standard VICReg (no disentanglement)")
            print(f"  - Backbone: {backbone}")
            print(f"  - Projection dim: {projection_dim}")
            print(f"  - VICReg weights: sim={sim_loss_weight}, var={var_loss_weight}, cov={cov_loss_weight}")
    ...
    def forward(self, x, active_group_idx=None, detach_inactive=True):
        ...
        Disentanglement design:
            - Active group: learns INVARIANCE via VICReg loss (updates backbone)
            - Inactive groups: learn SENSITIVITY via reversed loss (updates backbone)
            - Use --inactive_weight 2.0+ to let sensitivity compete with VICReg
        ...
        if self.use_disentanglement:
            ...
            # NOTE: VICReg requires UNNORMALIZED embeddings for variance/covariance losses
    ...
    def vicreg_loss(self, z_i, z_j):
        """
        VICReg loss: Variance + Invariance + Covariance.

        Components:
        1. Invariance: MSE between two views (encourages similar representations)
        2. Variance: Maintains variance above threshold (prevents collapse)
        3. Covariance: Decorrelates different dimensions (reduces redundancy)
        ...
        Returns:
            total_loss, sim_loss, var_loss, cov_loss
        """
        ...
        # === 2. Variance loss (hinge loss to maintain variance >= threshold) ===
        # Apply to both views
        std_z_i = torch.sqrt(z_i.var(dim=0) + 1e-04)
        std_z_j = torch.sqrt(z_j.var(dim=0) + 1e-04)
        var_loss = torch.mean(F.relu(self.variance_threshold - std_z_i))
        var_loss = var_loss + torch.mean(F.relu(self.variance_threshold - std_z_j))
        ...
        # Compute covariance matrices [dim, dim]
        cov_z_i = (z_i_centered.T @ z_i_centered) / (batch_size - 1)
        cov_z_j = (z_j_centered.T @ z_j_centered) / (batch_size - 1)

        # Sum of squared off-diagonal elements
        cov_loss = self._off_diagonal(cov_z_i).pow_(2).sum() / z_i.shape[1]
        cov_loss = cov_loss + self._off_diagonal(cov_z_j).pow_(2).sum() / z_j.shape[1]
    ...
    def _off_diagonal(self, x):
        """Return off-diagonal elements of a square matrix as a flattened tensor."""
        n, m = x.shape
        assert n == m
    ...
    def sensitivity_loss(self, z1_groups, z2_groups, active_idx):
        ...
        - Active group: INVARIANT to its aug (via VICReg)
        - Inactive groups: SENSITIVE to active aug (via this loss)

        Requires high --inactive_weight (e.g., 2.0) to compete with VICReg losses.
        """
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_disentanglement:
            ...
            # === LOSS 1: Active group VICReg (using single-aug views) ===
            loss_active, sim_active, var_active, cov_active = self.vicreg_loss(
                z1_groups[active_idx], z2_groups[active_idx]
            )

            # === LOSS 2: Semantic group VICReg (using ALL-aug views) ===
            semantic_idx = self.num_specific_groups  # Last group is semantic
            loss_semantic, sim_semantic, var_semantic, cov_semantic = self.vicreg_loss(
                z_sem1_groups[semantic_idx], z_sem2_groups[semantic_idx]
            )
            ...
            # Active group gets uniformity from VICReg (covariance term)
            # Semantic group gets uniformity from VICReg (L_semantic)
            ...
            # Primary loss for monitoring (sum of VICReg losses)
            ...
            # VICReg component losses
            self.log('train/sim_active', sim_active, on_step=False, on_epoch=True, batch_size=batch_size)
            self.log('train/var_active', var_active, on_step=False, on_epoch=True, batch_size=batch_size)
            self.log('train/cov_active', cov_active, on_step=False, on_epoch=True, batch_size=batch_size)
            ...
            total_loss, sim_loss, var_loss, cov_loss = self.vicreg_loss(z_i, z_j)

            self.log('train/loss', total_loss, on_step=True, on_epoch=True, prog_bar=True, batch_size=batch_size)
            self.log('train/sim_loss', sim_loss, on_step=False, on_epoch=True, batch_size=batch_size)
            self.log('train/var_loss', var_loss, on_step=False, on_epoch=True, batch_size=batch_size)
            self.log('train/cov_loss', cov_loss, on_step=False, on_epoch=True, batch_size=batch_size)

            with torch.no_grad():
                # Monitor variance statistics
                std_z_i = torch.sqrt(z_i.var(dim=0) + 1e-04)
                std_z_j = torch.sqrt(z_j.var(dim=0) + 1e-04)
                self.log('monitor/mean_std_i', std_z_i.mean(), on_step=False, on_epoch=True, batch_size=batch_size)
                self.log('monitor/mean_std_j', std_z_j.mean(), on_step=False, on_epoch=True, batch_size=batch_size)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.use_disentanglement:
            ...
            self.log('val/loss', total_loss, on_step=False, on_epoch=True, prog_bar=True, batch_size=batch_size)
            self.log('val/sim_loss', sim_loss, on_step=False, on_epoch=True, batch_size=batch_size)
            self.log('val/var_loss', var_loss, on_step=False, on_epoch=True, batch_size=batch_size)
            self.log('val/cov_loss', cov_loss, on_step=False, on_epoch=True, batch_size=batch_size)
